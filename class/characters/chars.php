...
// Checks to see if the character is online. returns TRUE if character is online
	...
	public function isOnline($guid)
    {
        ...
        $row = $this->mysql["char"]->mysql_count("SELECT COUNT(*) AS `count` FROM `characters` WHERE `guid` = '$guid' AND `online` = '1'");
        if($row['count'] > 0) 
		{
			return TRUE;
		}
...
// Checks if the character name exists or not
	...
	{
		$check = $this->mysql["char"]->selectRow("SELECT * FROM `characters` WHERE `name` LIKE '$name'");
		if ($check !== FALSE) 
		{
			return TRUE;
		}
    ...
    */
    public function addSpell($guid, $spell, $active = 1, $disabled = 0)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT COUNT(*) AS `count` FROM `character_spell` WHERE `guid` = '$guid' AND `spell` = '$spell'");
        $row = mysql_fetch_array($sql);
        if($row['count'] > 0) return false;

        $this->mysql["char"]->send("INSERT INTO `character_spell` (guid,spell,active,disabled) VALUES ('$guid','$spell','$active','$disabled')");
        return true;
    }
    ...
    */
    public function adjustLevel($guid, $mod)
    {
        ...
        if(PROJECT == "mangos")
        {
            $sql = $this->mysql["char"]->retrieve("SELECT `level` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
            $row = mysql_fetch_array($sql);
            $lvl = $row['level']+$adjust;
            $this->mysql["char"]->send("UPDATE `characters` SET `level` = '$lvl' WHERE `guid` = '$guid' LIMIT 1");
        } else {
            $sql = $this->mysql["char"]->retrieve("SELECT `data` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
            $row = mysql_fetch_array($sql);
            ...
            $this->mysql["char"]->send("UPDATE `characters` SET `data` = '$data' WHERE `guid` = '$guid' LIMIT 1");
        }
        return true;
    }
    ...
    */
    public function adjustMoney($guid, $mod)
    {
        ...
        if(PROJECT == "mangos")
        {
            $sql = $this->mysql["char"]->retrieve("SELECT `money` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
            $row = mysql_fetch_array($sql);
            $money = $row['money']+$adjust;
            $this->mysql["char"]->send("UPDATE `characters` SET `money` = '$money' WHERE `guid` = '$guid' LIMIT 1");
        } else {
            $sql = $this->mysql["char"]->retrieve("SELECT `data` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
            $row = mysql_fetch_array($sql);
            ...
            $imp = implode(' ',$exp);
            $this->mysql["char"]->send("UPDATE `characters` SET `data` = '$imp' WHERE `guid` = '$guid' LIMIT 1");
        }
        return true;
    }
    ...
    */
    public function getAccountId($guid)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT `account` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
        $row = mysql_fetch_array($sql);
    ...
    */
    public function getCharactersListFromAccountId($id)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT COUNT(*) AS `count` FROM `characters` WHERE `account` = '$id'");
        $row = mysql_fetch_array($sql);
        $result['0'] = $row['count'];

        $sql = $this->mysql["char"]->retrieve("SELECT `guid`, `name` FROM `characters` WHERE `account` = '$id'");
        $i = 1;
    ...
    */
    public function getClass($guid) {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT `class` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
        $row = mysql_fetch_array($sql);
        return $row['class'];
    }
    ...
    */
    ...
    {
        $guid = mysql_real_escape_string($guid);
        $sql = $this->mysql["char"]->retrieve("SELECT * FROM `character_inventory` WHERE `guid` = '$guid' AND `slot` < '19' AND `bag` = '0'");
        while($row = mysql_fetch_array($sql))
        {
            $slot = $row['slot'];
    ...
    */
    public function getFaction($guid)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT `race` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
        $row = mysql_fetch_array($sql);
    ...
    */
    public function getGender($guid)
    {
        ...
        if(PROJECT == "mangos")
            $sql = $this->mysql["char"]->retrieve("SELECT `gender` FROM `characters` WHERE guid = $guid LIMIT 1");
        else
            // took from mmfpm
            $sql = $this->mysql["char"]->retrieve("SELECT mid(lpad( hex( CAST(substring_index(substring_index(data,' ',".(DATA_FIELD_GENDER+1)."),' ',-1) as unsigned) ),8,'0'),4,1) as gender FROM `characters` WHERE guid = $guid LIMIT 1");
    ...
    */
    public function getGuid($name)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT `guid` FROM `characters` WHERE `name` = '$name' LIMIT 1");
        $row = mysql_fetch_array($sql);
    ...
    */
    public function getGuildRank($guid)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT `rname`, `rid` FROM `guild_rank` WHERE `rid` = '".$row['rank']."' AND `guildid` = '".$row['guildid']."' LIMIT 1");
        if(mysql_affected_rows() == 0) return false;
        $row = mysql_fetch_array($sql);
    ...
    */
    ...
    {
        $sql = $this->mysql["char"]->retrieve("SELECT `map`, `zone`, `position_x`, `position_y`, `position_z` FROM `character_homebind` WHERE `guid` = '$guid'");
    ...
    */
    public function getItemCount($guid, $item)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT `item` FROM `character_inventory` WHERE `guid` = '$guid' AND `item_template` = '$item'");
        $i = 0;
        while($row = mysql_fetch_array($sql))
        {
            $sqlb = $this->mysql["char"]->retrieve("SELECT `data` FROM `item_instance` WHERE `guid` = '$row[item]'");
    ...
    */
    public function getLevelMangos($guid)
    {
        ...
        $guid = mysql_real_escape_string($guid);
        $sql = $this->mysql["char"]->retrieve("SELECT `level` FROM `characters` WHERE `guid` = '$guid'");
        $row = mysql_fetch_array($sql);
        return $row['level'];
    }
    ...
    */
    ...
    {
        $guid = mysql_real_escape_string($guid);
        $sql = $this->mysql["char"]->retrieve("SELECT * FROM `character_inventory` WHERE `guid` = '$guid' AND `slot` > '22' AND `slot` < '39'");
        while($row = mysql_fetch_array($sql))
        {
    ...
    */
    public function getMoneyMangos($guid)
    {
        ...
        $guid = mysql_real_escape_string($guid);
        $sql = $this->mysql["char"]->retrieve("SELECT `money` FROM `characters` WHERE `guid` = '$guid'");
        $row = mysql_fetch_array($sql);
        return $row['money'];
    }
    ...
    */
    ...
    {
        $guid = mysql_real_escape_string($guid);
        $sql = $this->mysql["char"]->retrieve("SELECT `name` FROM `characters` WHERE `guid` = '$guid' LIMIT 1");
        $row = mysql_fetch_array($sql);
        return $row['name'];
    }
    ...
    */
    ...
    {
        $sql = $this->mysql["char"]->retrieve("SELECT COUNT(*) AS `count` FROM `characters` WHERE `online` = '1'");
        $row = mysql_fetch_array($sql);
    ...
    */
    ...
    {
        $sql = $this->mysql["char"]->retrieve("SELECT COUNT(*) AS `count` FROM `characters` WHERE `guid` = '$guid' AND `online` = '1'");
        $row = mysql_fetch_array($sql);
    ...
    */
    public function removeSpell($guid, $spell)
    {
        $guid = mysql_real_escape_string($guid);
        $spell = mysql_real_escape_string($spell);
        $this->mysql["char"]->send("DELETE FROM `character_spell` WHERE `guid` = '$guid' AND `spell` = '$spell'");
        return true;
    }
    ...
    */
    ...
    {
        $guid = mysql_real_escape_string($guid);
        $this->mysql["char"]->send("DELETE FROM `character_aura` WHERE `guid` = '".$guid."' AND `spell` = '20584' OR `guid` = '".$guid."' AND `spell` = '8326'");
        return true;
    }
    ...
    */
    public function setAccountId($guid,$accountId)
    {
        ...
        $this->mysql["char"]->send("UPDATE `characters` SET `account` = '$acct' WHERE `guid` = '$guid' LIMIT 1");
        return true;
    }
    ...
    */
    public function setLevel($guid, $newlevel)
    {
        ...
        if(PROJECT != "mangos")
            ...
            $this->mysql["char"]->send("UPDATE `characters` SET `level` = '$newlevel' WHERE `guid` = '$guid' LIMIT 1");
    ...
    */
    public function setMoney($guid, $sumofmoney)
    {
        ...
        if(PROJECT != "mangos")
            ...
            $this->mysql["char"]->send("UPDATE `characters` SET `money` = '$sum' WHERE `guid` = '$guid' LIMIT 1");
    ...
    */
    public function setName($guid,$newname)
    {
        ...
        $sql = $this->mysql["char"]->retrieve("SELECT COUNT(*) AS `count` FROM `characters` WHERE `name` = '$newname' LIMIT 1");
        $row = mysql_fetch_array($sql);
        if($row['count'] > 0) return false;
        $this->mysql["char"]->send("UPDATE `characters` SET `name` = '$newname' WHERE `guid` = '$guid' LIMIT 1");
        return true;
    }
	...
	function getOnlineCount($faction = 0)
	{
		...
		if($faction == 0)
		{
			$count = $fnmaDB["char"]->mysql_count("SELECT COUNT(*) FROM `characters` WHERE `online`='1'");
		}
		elseif($faction == 1)
		{
			$count = $fnmaDB["char"]->mysql_count("SELECT COUNT(*) FROM `characters` WHERE `online`='1' AND (`race` = 1 OR `race` = 3 OR `race` = 4 OR `race` = 7 OR `race` = 11)");
		}
		elseif($faction == 2)
		{
			$count = $fnmaDB["char"]->mysql_count("SELECT COUNT(*) FROM `characters` WHERE `online`='1' AND (`race` = 2 OR `race` = 5 OR `race` = 6 OR `race` = 8 OR `race` = 10)");
		}
		return $count;
	}
	...
	function getOnlineList($faction = 0, $start = 0, $limit = 10000)
	{
		...
		if($faction == 0)
		{
			$list = $fnmaDB["char"]->select("SELECT guid, name, race, class, gender, level, zone  FROM `characters` WHERE `online`='1' LIMIT $start, $limit");
		}
		elseif($faction == 1)
		{
			$list = $fnmaDB["char"]->select("SELECT guid, name, race, class, gender, level, zone  FROM `characters` WHERE `online`='1' AND 
	...
	function getFactionTopKills($faction, $count)
	{
		...
		if($faction == 1)
		{			
			$row = $fnmaDB["char"]->select("SELECT * FROM `characters` WHERE `totalkills` > 0 AND (
				`race` = 1 OR `race` = 3 OR `race` = 4 OR `race` = 7 OR `race` = 11
